...
"""
ref: https://blog.lxsang.me/post/id/16
"""
...
def callback(data):
    ...
    if last_odom == None:
        ...
        # calculate new odometry pose by using the motion model
...
if __name__ == '__main__':
    ...
    rospy.init_node('noise_odom_pub', anonymous=True)
    ...
    if rospy.has_param("~old_odom_topic"):
        old_odom_topic = rospy.get_param("~old_odom_topic")
    else:
        rospy.logerror("old_odom_topic is not setup")

    # -- get old odom topic
    if rospy.has_param("~new_odom_topic"):
        new_odom_topic = rospy.get_param("~new_odom_topic")
    else:
        rospy.logerror("new_odom_topic is not setup")
    ...
    # -- get tf_odom_enabled
    if rospy.has_param("~tf_odom_enabled"):
        tf_odom_enabled = rospy.get_param("~tf_odom_enabled")
    else:
        tf_odom_enabled = True
        rospy.logwarn("tf_odom_enabled is set to True")

    publish = rospy.Publisher(name=new_odom_topic, data_class=Odometry, queue_size=10)
    rospy.Subscriber(name=old_odom_topic, data_class=Odometry, callback=callback)
